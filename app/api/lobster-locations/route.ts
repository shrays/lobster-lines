...
function getTimezone(zip: string): string {
  ...
  if (isNaN(parseInt(zip.charAt(0)))) {
    for (var timezone in canadaTimezoneMapping) {
      if (
        canadaTimezoneMapping[timezone].some((prefix: string) =>
...
async function fetchWaitTime(rlid: string, zip: string): Promise<number> {
  try {
    ...
    const firstAvailable = slots.find(
      (slot) => slot.isAvailable && !slot.onlineSuspended,
    );
    ...
    const diffMinutes =
      (availableTime.getTime() - requestedTime.getTime()) / 60000;
...
export async function GET(req: NextRequest) {
  ...
  const locationsWithStatus = data.locations.map((item: any) => {
    ...
    const open =
      !temporarilyClosed &&
      isOpen(loc.zip, loc.hours[dayOfWeek].open, loc.hours[dayOfWeek].close);
  ...
  const waitTimes = await processInBatches(
    openLocations,
    CONCURRENCY_LIMIT,
    (loc: any) => fetchWaitTime(loc.rlid, loc.zip),
  );
  ...
  const openStores = simplifiedData.filter(
    (item: Location) =>
      item.estimatedWaitTime !== -1 && item.estimatedWaitTime !== -2,
  );
  const storesOpen = openStores.length;
  const storesWithWaitlist = openStores.filter(
    (item: Location) => item.estimatedWaitTime > 0,
  ).length;
  ...
  const averageWaitTime =
    storesOpen > 0 ? totalWaitTimeForOpenStores / storesOpen : 0;
